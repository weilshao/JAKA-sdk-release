...
#define PI 3.1415926
//4.1~4.9机器人手动运动
std::string ipaddr = "192.168.2.149";
int example_jog()
{
	//实例API对象demo 
	JAKAZuRobot demo;
...
int example_linear_move()
{
	...
	CartesianPose cart;
	cart.tran.x = 0; cart.tran.y = 0; cart.tran.z = 50;
	cart.rpy.rx = 109 * PI / 180; cart.rpy.ry = 90 * PI / 180; cart.rpy.rz = 0 * PI / 180;
	//demo.set_tool_id(2);
...
int example_servo_j()
{
	...
	JointValue joint_pos2 = { 0.001, 0 * PI / 180, 0 * PI / 180, 0 * PI / 180, 0 * PI / 180, 0.001 };
	int n = 0;
	while (n<1000) {
		for (int i = 0; i < 100; i++)
		{
...
int example_get_robot_status()
{
	...
	std::cout << " extio is : {";
	for (int i = 1; i < (sizeof(robstatus.extio) / sizeof(robstatus.extio[0])); i++)
	{	
		std::cout << robstatus.extio[i] << ",";
		if (0 == i % 20)
			std::cout << std::endl;
	}
	std::cout << robstatus.extio[sizeof(robstatus.extio) / sizeof(robstatus.extio[0]) - 1] << "}" << std::endl;
	std::cout << " din is : {";
...
//		demo.servo_move_enable(FALSE);
//		//定义并初始化关JointValue变量
//		JointValue joint_pos = { 0, 90 * PI / 180, 0 * PI / 180, 0 * PI / 180, 0 * PI / 180, 0 * PI / 180 };
//		JointValue joint_pos2 = { 90 * PI / 180, 90 * PI / 180, 0 * PI / 180, 0 * PI / 180, 0 * PI / 180, 0 * PI / 180 };
//		double dur1,dur2;
//		clock_t start1, end1,start2,end2;
//		start1 = clock();
//		demo.joint_move(&joint_pos2, ABS, TRUE, PI, PI, 0, nullptr);
//		end1 = clock();
//		dur1 = (double)(end1 - start1);
//		demo.joint_move(&joint_pos2, ABS, TRUE, PI, PI, 0, nullptr);
//		start2 = clock();
//		demo.joint_move(&joint_pos, ABS, TRUE, PI, PI, 0, nullptr);
//		end2 = clock();
//		printf("Use Time:%f\n", (dur1 / CLOCKS_PER_SEC));
//		if ((dur1 / CLOCKS_PER_SEC) < 1.0)
//			break;
//		dur2 = (double)(end2 - start2);
//		printf("Use Time:%f\n", (dur2 / CLOCKS_PER_SEC));
//		if ((dur2 / CLOCKS_PER_SEC) < 1.0)
//			break;
//		//TRUE为进入伺服模式
//		std::cin >> x;
//	}	
