...
typedef struct
{
	int din[256];				  ///< 外部数字输入din[0]为有效信号的个数
	int dout[256];				  ///< 外部数字输出dout[0]为有效信号的个数
	int ain[256];				  ///< 外部模拟输入ain[0]为有效信号的个数
	int aout[256];			      ///< 外部模拟输出aout[0]为有效信号的个数
} Extio;
...
typedef struct
{
	...
	int dout[1024];									///< 机器人控制柜数字输出信号,dout[0]为信号的个数
	int tio_dout[1024];								///< 机器人末端工具数字输出信号,tio_dout[0]为信号的个数
	Extio extio;								    ///< 机器人外部应用IO
	int din[1024];									///< 机器人控制柜数字输入信号,din[0]为信号的个数
	int tio_din[1024];								///< 机器人末端工具数字输入信号,tio_din[0]为信号的个数
	double ain[1024];								///< 机器人控制柜模拟输入信号,ain[0]为信号的个数
	double tio_ain[1024];							///< 机器人末端工具模拟输入信号,tio_ain[0]为信号的个数
	double aout[1024];								///< 机器人控制柜模拟输出信号,aout[0]为信号的个数
